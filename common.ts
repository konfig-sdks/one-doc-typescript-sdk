...
import { readableStreamToString, OneDocError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new OneDocError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new OneDocError(e, e.response?.data)
                }
                if (innerError instanceof OneDocError) {
                    // Got "OneDocError" from the above try block
                    throw innerError;
                }
